(() => {
  ...
  const applyCopy = () => {
    ...
    const heroName = document.querySelector('.featured-profile .profile-overlay h2');
    if (heroName) heroName.textContent = `${t.name}, 41`;

    const messageName = elements.miniMessage.querySelector('strong');
    const messageText = elements.miniMessage.querySelector('[data-i18n="messagePreview"]');
    if (messageName) {
    ...
    const sticky = document.querySelector('.mobile-sticky [data-i18n="stickyCta"], .mobile-sticky span:first-child');
    if (sticky) sticky.textContent = t.cta;
  };
  ...
  const initialise = () => {
    ...
    const select = document.getElementById('languageSelect');
    select?.addEventListener('change', () => setTimeout(applyCopy, 0));
