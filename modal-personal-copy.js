(() => {
  const localeCopy = {
    ...
    bg: {
      badge: 'ПОСЛЕДНА СТЪПКА',
      title: (name) => `${name} може да получава съобщения само от потвърдени потребители`,
      confirm: 'Потвърди възрастта си, за да можеш да ѝ отговориш.',
      note: 'В следващата стъпка ще създадеш кратък профил и ще въведеш имейл адрес. Отнема само момент.',
      ageYoung: 'На 18–44 години съм',
      ageOlder: 'На 45+ години съм',
      profileStep: (name) => `Профил · ${name}`,
      verifyStep: 'Проверка',
      privacyAria: 'Дискретен достъп',
      profileAria: 'Кратък профил',
      verifiedAria: 'Потвърден достъп',
      accountCta: 'Отговори на Мария'
    },
    ro: {
      badge: 'ULTIMUL PAS',
      title: (name) => `${name} poate primi mesaje doar de la utilizatori verificați`,
      confirm: 'Confirmă-ți vârsta ca să îi poți răspunde.',
      note: 'La pasul următor îți creezi un profil scurt și introduci adresa de e-mail. Durează doar un moment.',
      ageYoung: 'Am 18–44 de ani',
      ageOlder: 'Am 45+ ani',
      profileStep: (name) => `Profil · ${name}`,
      verifyStep: 'Verificare',
      privacyAria: 'Acces discret',
      profileAria: 'Profil scurt',
      verifiedAria: 'Acces verificat',
      accountCta: 'Răspunde-i Mariei'
    },
    et: {
      badge: 'VIIMANE SAMM',
      title: (name) => `${name} saab sõnumeid vastu võtta ainult kinnitatud kasutajatelt`,
      confirm: 'Kinnita oma vanus, et saaksid talle vastata.',
      note: 'Järgmises sammus lood lühikese profiili ja sisestad oma e-posti aadressi. See võtab vaid hetke.',
      ageYoung: 'Olen 18–44',
      ageOlder: 'Olen 45+',
      profileStep: (name) => `Profiil · ${name}`,
      verifyStep: 'Kinnitamine',
      privacyAria: 'Diskreetne ligipääs',
      profileAria: 'Lühike profiil',
      verifiedAria: 'Kinnitatud ligipääs',
      accountCta: 'Vasta Mariale'
    },
    lt: {
      badge: 'PASKUTINIS ŽINGSNIS',
      title: (name) => `${name} gali gauti žinutes tik iš patvirtintų naudotojų`,
      confirm: 'Patvirtink savo amžių, kad galėtum jai atsakyti.',
      note: 'Kitame žingsnyje susikursi trumpą profilį ir įvesi el. pašto adresą. Tai užtruks tik akimirką.',
      ageYoung: 'Man 18–44',
      ageOlder: 'Man 45+',
      profileStep: (name) => `Profilis · ${name}`,
      verifyStep: 'Patvirtinimas',
      privacyAria: 'Diskretiška prieiga',
      profileAria: 'Trumpas profilis',
      verifiedAria: 'Patvirtinta prieiga',
      accountCta: 'Atsakyk Marijai'
    },
    lv: {
      badge: 'PĒDĒJAIS SOLIS',
      title: (name) => `${name} var saņemt ziņas tikai no verificētiem lietotājiem`,
      confirm: 'Apstiprini savu vecumu, lai varētu viņai atbildēt.',
      note: 'Nākamajā solī izveidosi īsu profilu un ievadīsi e-pasta adresi. Tas aizņem tikai mirkli.',
      ageYoung: 'Man ir 18–44',
      ageOlder: 'Man ir 45+',
      profileStep: (name) => `Profils · ${name}`,
      verifyStep: 'Pārbaude',
      privacyAria: 'Diskrēta piekļuve',
      profileAria: 'Īss profils',
      verifiedAria: 'Pārbaudīta piekļuve',
      accountCta: 'Atbildi Marijai'
    },
    uk: {
      badge: 'ОСТАННІЙ КРОК',
      title: (name) => `${name} може отримувати повідомлення лише від перевірених користувачів`,
      confirm: 'Підтвердь свій вік, щоб відповісти їй.',
      note: 'На наступному кроці ти створиш короткий профіль і введеш адресу електронної пошти. Це займе лише мить.',
      ageYoung: 'Мені 18–44',
      ageOlder: 'Мені 45+',
      profileStep: (name) => `Профіль · ${name}`,
      verifyStep: 'Перевірка',
      privacyAria: 'Дискретний доступ',
      profileAria: 'Короткий профіль',
      verifiedAria: 'Перевірений доступ',
      accountCta: 'Відповісти Марії'
    },
    he: {
      badge: 'שלב אחרון',
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      #ageGateModal.rmc-age-gate-v3 .age-gate-badge{padding:6px 10px!important;border-color:rgba(239,101,150,.42)!important;background:rgba(239,101,150,.12)!important;color:#ff96b8!important;font-size:.67rem!important;font-weight:950!important;letter-spacing:.105em!important}
      ...
      #ageGateModal.rmc-age-gate-v3 .rmc-progress-track::before{content:'';position:absolute;top:50%;left:25%;right:25%;height:2px;transform:translateY(-50%);background:linear-gradient(90deg,#ef6596,#d44f82);box-shadow:0 0 10px rgba(239,101,150,.22)}
      ...
      #ageGateModal.rmc-age-gate-v3 .rmc-trust-icon:last-child{color:#ff91b7;border-color:rgba(239,101,150,.20);background:rgba(239,101,150,.06)}
...
(() => {
  ...
  const timerCopy = {
    ...
    bg: { label: 'Тази покана изтича окончателно след', expired: 'Поканата ти изтече', renew: 'Поднови поканата' },
    ro: { label: 'Această invitație expiră definitiv în', expired: 'Invitația ta a expirat', renew: 'Reînnoiește invitația' },
    et: { label: 'See kutse aegub lõplikult', expired: 'Sinu kutse aegus', renew: 'Uuenda kutset' },
    lt: { label: 'Šis kvietimas galutinai baigsis po', expired: 'Tavo kvietimas baigėsi', renew: 'Atnaujinti kvietimą' },
    lv: { label: 'Šis uzaicinājums neatgriezeniski beigsies pēc', expired: 'Tavs uzaicinājums ir beidzies', renew: 'Atjaunot uzaicinājumu' },
    uk: { label: 'Це запрошення остаточно закінчиться через', expired: 'Термін запрошення минув', renew: 'Оновити запрошення' },
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      .rmc-invite-countdown{position:relative;display:grid;grid-template-columns:34px minmax(0,1fr) auto;align-items:center;gap:11px;margin:0 0 9px;padding:13px 14px;border:1px solid rgba(255,137,177,.34);border-radius:16px;background:linear-gradient(135deg,rgba(239,101,150,.16),rgba(255,255,255,.025));box-shadow:inset 0 1px 0 rgba(255,255,255,.04),0 9px 24px rgba(0,0,0,.18);color:#fff;overflow:hidden}
      .rmc-invite-countdown::before{content:'';position:absolute;inset:0 auto 0 0;width:3px;background:linear-gradient(#ff4a55,#9d3e6a)}
      .rmc-invite-countdown-icon{display:grid;width:34px;height:34px;place-items:center;border-radius:11px;background:rgba(239,101,150,.16);font-size:1rem;box-shadow:inset 0 0 0 1px rgba(255,145,183,.16)}
      ...
      .rmc-invite-countdown-time{min-width:84px;color:#fff;font:950 1.65rem/1 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-variant-numeric:tabular-nums;letter-spacing:.025em;text-align:right;text-shadow:0 0 22px rgba(239,101,150,.22)}
      .rmc-invite-countdown.is-urgent{border-color:rgba(255,149,186,.68);background:linear-gradient(135deg,rgba(239,101,150,.24),rgba(255,255,255,.03))}
      .rmc-invite-countdown.is-urgent .rmc-invite-countdown-time{animation:rmcCountdownPulse 1.1s ease-in-out infinite}
      .rmc-invite-countdown-renew{display:none;align-items:center;justify-content:center;min-height:40px;padding:8px 13px;border:0;border-radius:11px;background:linear-gradient(135deg,#ef6596,#b94373);box-shadow:0 8px 20px rgba(239,101,150,.24);color:#fff;font:850 .76rem/1.1 inherit;white-space:nowrap;cursor:pointer}
      .rmc-invite-countdown-renew:focus-visible{outline:3px solid rgba(255,255,255,.9);outline-offset:2px}
      .rmc-invite-countdown.is-expired{grid-template-columns:34px minmax(0,1fr) auto;border-color:rgba(255,141,181,.52);background:linear-gradient(135deg,rgba(239,101,150,.20),rgba(255,255,255,.025))}
