...
export async function onRequest({ request, params }) {
  const market = String(params.market || '').toLowerCase();
  ...
  if (!title || profile !== `maria${market}`) {
    ...
    return Response.redirect(new URL(`/${fallbackMarket}/post`, request.url), 302);
  }

  const requestUrl = new URL(request.url);
  const canonical = `${requestUrl.origin}/${market}/profile/${profile}`;
